<?php
/**
 * @type $beautify \Nova\Exception\Beautify
 */
?>
...
<head>
    ...
    <!-- Framework icon -->
    <link rel="icon" href="<?php echo baseUrl('/nova_icon/favicon.png'); ?>" sizes="32x32">
    ...
    <style>
        ...
        tr:hover {
            cursor: url(http://localhost/novaframe/nova_icon/icon.png) 10 10, pointer;
        }
        ...
        .sub-container .body::-webkit-scrollbar {
            display: none;
        }
...
<section>
    ...
    <div class="container">
        <div class="box">
            <p><?php echo $beautify->title() ?></p>
        </div>

        <div class="message">
            <p><?php echo $beautify->message() ?></p>
        </div>

        <div>
            <?php $beautify->display('messages') ?>
        </div>

        <div class="sub-container">
            <div class="nav">
                <ul>
                    <li class="nav-backtrace active" onclick="PageManagement('backtrace')">Backtrace</li>
                    <li class="disable">|</li>
                    <li class="nav-file" onclick="PageManagement('file')">Files</li>
                </ul>
            </div>
            ...
            <div class="body">
                ...
                <div class="file page">
                    <?php $prepared = []; $base = config('app.base_path'); foreach (array_reverse(get_included_files()) as $file) : ?>
                        <?php if (!str_contains($file, 'Exception') && !str_contains($file, 'exception')) : ?>
                            <?php
                                ...
                                $str = str_replace($base, '', $file);
                                $arr = \Nova\Helpers\Modules\Str::toArray("\\", $str);
                                $path = $arr[0];
                                $prepared[$path][] = $file;
                            ?>
                        <?php endif; ?>
                    ...
                    <?php if (!empty($prepared)) : ?>
                        <?php foreach ($prepared as $p => $arr) : ?>
                            <h4><?php echo $p; ?></h4>
                            <ul>
                            <?php foreach ($arr as $index => $f) : ?>
                                <li><?php echo $f; ?></li>
                            <?php endforeach; ?>
                            </ul>
                        <?php endforeach; ?>
